...
Production version with Azure Blob Storage, Parquet format, and GitHub Actions compatibility.
...
def should_run_forecast(logger=None) -> Tuple[bool, str]:
    ...
    try:
        ...
        existing_data = download_from_blob(HISTORICAL_DATA_BLOB, 'parquet', logger)
...
def catch_up_missing_data(logger=None) -> bool:
    ...
    try:
        ...
        if len(new_data) > 0:
            ...
            append_to_blob_parquet(new_data_df, HISTORICAL_DATA_BLOB, logger)
...
def perform_health_check(logger=None):
    ...
    try:
        ...
        historical_data = download_from_blob(HISTORICAL_DATA_BLOB, 'parquet', logger)
...
System Status: {final_health['status']}
        ...
        current_data = download_from_blob(HISTORICAL_DATA_BLOB, 'parquet', logger)
        ...
        if current_data is not None and len(current_data) > 0:
            ...
            # Upload raw data to blob storage as parquet
            df_raw = df_data.reset_index().rename(columns={'index': 'Date'})
            upload_to_blob(df_raw, HISTORICAL_DATA_BLOB, 'parquet', logger)
        ...
        if daily_pred is not None:
            append_to_blob_parquet(daily_pred, PREDICTED_DATA_BLOB, logger)
        else:
        ...
        df_predicted_historical = download_from_blob(PREDICTED_DATA_BLOB, 'parquet', logger)
        ...
        if df_predicted_historical is not None and len(df_predicted_historical) > 0:
            ...
            append_to_blob_parquet(metrics_df, METRICS_BLOB, logger)
        else:
